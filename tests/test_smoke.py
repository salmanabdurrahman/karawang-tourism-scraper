...
class EntryPointSmokeTest(unittest.TestCase):
    ...
    def test_scrape_all_reviews_missing_input(self):
        with mock.patch.object(gmaps_reviews_scraper, "INPUT_FILE", MISSING), redirect_stdout(io.StringIO()) as buf:
    ...
    def test_scrape_images_only_missing_input(self):
        with mock.patch.object(gmaps_image_scraper, "INPUT_FILE", MISSING), redirect_stdout(io.StringIO()) as buf:
            result = gmaps_image_scraper.scrape_images_only()
        self.assertIsNone(result)
        self.assertIn("not found", buf.getvalue().lower())

    def test_process_all_files_missing_input_dir(self):
        with mock.patch.object(process_gmaps_data, "INPUT_DIR", MISSING), redirect_stdout(io.StringIO()):
            process_gmaps_data.process_all_files()  # must not raise

    def test_merge_data_missing_input(self):
        with mock.patch.object(merge_images_to_final, "MAIN_DATASET_FILE", MISSING), redirect_stdout(io.StringIO()):
            merge_images_to_final.merge_data()  # must not raise

    def test_process_data_missing_input_dir(self):
        with mock.patch.object(prepare_content_based, "INPUT_DIR", MISSING), redirect_stdout(io.StringIO()):
